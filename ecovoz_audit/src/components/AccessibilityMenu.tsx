...
export default function AccessibilityMenu({
  ...
  return (
    <>
      ...
      <div className="fixed right-4 sm:right-6 bottom-[84px] z-50 flex flex-col items-end gap-2">
        <button type="button"
          onClick={() => {
            setIsOpen(!isOpen);
            announceToScreenReader(isOpen ? "Menu de acessibilidade fechado" : "Menu de acessibilidade aberto. Pressione tab para navegar pelas opções.");
      ...
      {isOpen && (
        <div 
          ...
          className="fixed inset-y-0 right-0 z-[60] w-80 md:w-96 bg-white shadow-2.5xl border-l border-[#DDE8D8] p-6 flex flex-col overflow-y-auto animate-fade-in text-left transition-all"
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          <div className="space-y-6 mt-6 flex-1">
                ...
                <button type="button"
                  ...
                  className={`py-2 px-1 text-center font-semibold rounded-xl text-xs border transition-all ${
                    contrast === 'normal'
                      ...
                      : 'bg-white text-gray-700 border-[#DDE8D8] hover:bg-gray-50'
                  }`}
            ...
            <div className="space-y-2">
              ...
              <div className="bg-[#f5f7f6] p-2 rounded-xl flex items-center justify-between border border-[#DDE8D8]">
            ...
            <div className="flex items-center justify-between py-2 border-b border-gray-100">
              ...
              <button type="button"
                ...
                className={`relative inline-flex h-8 w-14 shrink-0 items-center rounded-full border-2 transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-[#F28C28]/35 ${
                  speechSynthesisEnabled ? 'bg-[#123524] border-[#123524]' : 'bg-white border-[#94A89B]'
                }`}
                role="switch"
                aria-checked={speechSynthesisEnabled}
                aria-label="Ativar leitura por síntese de voz"
              >
                <div className={`h-6 w-6 rounded-full shadow-md transition-transform ${
                  speechSynthesisEnabled ? 'translate-x-6 bg-[#A8CBB1]' : 'translate-x-0 bg-[#4B5F55]'
                }`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between py-2 border-b border-gray-100">
              ...
              <button type="button"
                ...
                className={`relative inline-flex h-8 w-14 shrink-0 items-center rounded-full border-2 transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-[#F28C28]/35 ${
                  underlineLinks ? 'bg-[#123524] border-[#123524]' : 'bg-white border-[#94A89B]'
                }`}
                role="switch"
              ...
              >
                <div className={`h-6 w-6 rounded-full shadow-md transition-transform ${
                  underlineLinks ? 'translate-x-6 bg-[#A8CBB1]' : 'translate-x-0 bg-[#4B5F55]'
                }`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between py-2 border-b border-gray-100">
              ...
              <button type="button"
                ...
                className={`relative inline-flex h-8 w-14 shrink-0 items-center rounded-full border-2 transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-[#F28C28]/35 ${
                  dyslexiaFont ? 'bg-[#123524] border-[#123524]' : 'bg-white border-[#94A89B]'
                }`}
                role="switch"
              ...
              >
                <div className={`h-6 w-6 rounded-full shadow-md transition-transform ${
                  dyslexiaFont ? 'translate-x-6 bg-[#A8CBB1]' : 'translate-x-0 bg-[#4B5F55]'
                }`} />
              </button>
            </div>
            ...
            <div className="flex items-center justify-between py-2 border-b border-gray-100">
              ...
              <button type="button"
                ...
                className={`relative inline-flex h-8 w-14 shrink-0 items-center rounded-full border-2 transition-all focus:outline-none focus-visible:ring-4 focus-visible:ring-[#F28C28]/35 ${
                  readingGuide ? 'bg-[#123524] border-[#123524]' : 'bg-white border-[#94A89B]'
                }`}
                role="switch"
              ...
              >
                <div className={`h-6 w-6 rounded-full shadow-md transition-transform ${
                  readingGuide ? 'translate-x-6 bg-[#A8CBB1]' : 'translate-x-0 bg-[#4B5F55]'
                }`} />
              </button>
            </div>

            {/* Section: Keyboard shortcut helper */}
            <div className="bg-[#f5f7f6] p-[18px] rounded-2xl border border-[#DDE8D8] space-y-3">
              <button type="button"
                ...
                <div className="space-y-2.5 pt-2 border-t border-gray-200 animate-fade-in text-xs max-h-48 overflow-y-auto custom-scrollbar">
                  {shortcuts.map((sc) => (
                    <div key={sc.key} className="flex justify-between items-start gap-1 pb-1.5 border-b border-gray-100 last:border-0">
                      <kbd className="px-1.5 py-0.5 bg-white border border-[#DDE8D8] rounded shadow-xs text-[9.5px] font-mono text-gray-800 shrink-0 font-bold">
                        {sc.key}
                      </kbd>
