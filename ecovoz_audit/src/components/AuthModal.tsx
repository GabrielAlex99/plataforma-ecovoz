...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }: AuthModalProps) {
  ...
  const handleForgotSubmit = (e: React.FormEvent) => {
    ...
    // Fluxo demonstrativo: em produção, esta ação chamaria um serviço seguro de recuperação.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Get existing users list from local demo storage
    let users: any[] = [];
    try {
  ...
  return (
    <div className="fixed inset-0 z-55 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-labelledby="auth-modal-title" className="bg-white border border-[#e9ecef] rounded-[28px] max-w-md w-full max-h-[85vh] md:max-h-[90vh] overflow-y-auto custom-scrollbar p-6 md:p-8 shadow-2xl relative text-left animate-scale-up">
        
        {/* Close Button */}
        <button 
          type="button"
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-xl bg-[#f5f7f6] text-gray-400 hover:text-[#4B5F55] border border-[#e9ecef] transition-all cursor-pointer"
          title="Fechar"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <div className="space-y-6">
          ...
          <div className="text-center">
            ...
            <h3 id="auth-modal-title" className="font-sans font-bold text-2xl text-[#1b4332] tracking-tight">
              {isForgotMode 
                ? 'Recuperar Senha' 
            ...
            <p className="text-xs text-gray-500 mt-1 max-w-xs mx-auto">
              {isForgotMode
                ? 'Informe o e-mail cadastrado para simular o fluxo seguro de recuperação de acesso.'
                : (isLogin 
          ...
          {error && (
            <div className="bg-rose-50 border border-rose-200 p-3 rounded-xl text-xs text-rose-700 flex items-center space-x-2 animate-pulse">
          ...
          {isForgotMode ? (
            <form onSubmit={handleForgotSubmit} className="space-y-4">
              {recoverySuccess ? (
                <div className="bg-emerald-50 border border-emerald-200 p-5 rounded-2xl text-xs text-emerald-800 text-center space-y-3 animate-scale-up">
                  ...
                  <p className="font-bold text-sm">Solicitação de recuperação registrada</p>
                  <p className="leading-relaxed text-emerald-700">
                    Neste protótipo, a solicitação foi registrada para o e-mail: <br/>
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label htmlFor="auth-recovery-email" className="text-xs font-semibold text-gray-500 uppercase block">E-mail Cadastrado</label>
                    <div className="relative">
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-[#e9ecef] text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#1b4332] text-sm transition-colors shadow-xs"
                      />
                  ...
                  >
                    Registrar solicitação de recuperação
                  </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!isLogin && (
                <div className="space-y-1.5">
                  <label htmlFor="auth-name" className="text-xs font-semibold text-gray-500 uppercase block">Qual o seu nome?</label>
                  <div className="relative">
                    <input
                      id="auth-name"
                      type="text"
              ...
              {/* Email Input */}
              <div className="space-y-1.5">
                <label htmlFor="auth-email" className="text-xs font-semibold text-gray-500 uppercase block">E-mail corporativo / pessoal</label>
                <div className="relative">
                  <input
                    id="auth-email"
                    type="email"
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  <label htmlFor="auth-password" className="text-xs font-semibold text-gray-500 uppercase block">Senha de acesso</label>
                  {isLogin && (
                    <button
                      type="button"
                      onClick={() => {
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 rounded-xl bg-white border border-[#e9ecef] text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#1b4332] text-sm transition-colors shadow-xs"
                  />
                  ...
                  <button
                    ...
                    className="absolute right-3.5 top-3 text-[#4B5F55] hover:text-gray-700 transition-colors cursor-pointer"
                  >
              ...
              {!isLogin && (
                <div className="flex items-start space-x-2.5 text-xs text-[#4B5F55] bg-[#1b4332]/5 p-3.5 rounded-xl border border-[#1b4332]/10 mt-4 shadow-2xs">
                  <input
                    id="lgpd-signup"
                    type="checkbox"
                    checked={lgpdConsent}
          ...
          <div className="pt-4 border-t border-[#e9ecef] text-center">
            <p className="text-xs text-[#4B5F55]">
