...
export default function DashboardESG({ relatos, propostas, currentUser }: DashboardESGProps) {
  ...
  useEffect(() => {
    const updateChartLayout = () => setIsMobileChart(window.innerWidth < 640);
    updateChartLayout();
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          >
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={categoryChartData} layout="vertical" margin={{ top: 10, right: isMobileChart ? 8 : 24, left: isMobileChart ? -18 : 8, bottom: 8 }}>
                <CartesianGrid stroke="#DDE8D8" strokeDasharray="3 3" />
                <XAxis type="number" tick={{ fill: '#4B5F55', fontSize: 11 }} allowDecimals={false} />
                <YAxis
                  type="category"
                  dataKey="name"
                  width={isMobileChart ? 82 : 110}
                  tick={{ fill: '#4B5F55', fontSize: 11 }}
                  tickFormatter={(value) => isMobileChart && String(value).length > 11 ? `${String(value).slice(0, 10)}…` : String(value)}
                />
                <Tooltip contentStyle={tooltipStyle} />
