...
Relato: "${text}"`,
  ...
  app.post("/api/enviar-email-esg", (req, res) => {
    try {
      ...
      console.log("\n======================================================================");
      console.log(`📡 SIMULAÇÃO DE NOTIFICAÇÃO ESG - DESTINATÁRIO DEMONSTRATIVO: ${targetEmail}`);
      ...
      if (tipo === "Relato") {
        ...
        console.log(`Corpo do E-mail:\n`);
        console.log(`Olá, coordenador ESG Galexander Santos,\n`);
        console.log(`Uma nova proposta ou solução foi registrada para pauta e discussão.`);
        console.log(`\n--- DADOS DA PROPOSTA ---`);
        console.log(`ID: ${data.id}`);
      ...
      return res.json({ 
        success: true, 
        message: `Registro de notificação simulado com sucesso para ${targetEmail}`,
        recipient: targetEmail,
        timestamp: new Date().toISOString()
      });

    } catch (err: any) {
      console.error("Erro ao registrar simulação de e-mail:", err);
      res.status(500).json({ error: "Erro interno ao registrar simulação de e-mail." });
    }
  });
